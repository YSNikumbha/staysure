...
export default function OwnerBookingsPage() {
  ...
  const handleApprove = async (id: number) => {
    ...
    try {
      await bookingApi.ownerApprove(id);
      await loadBookings();
    } catch (err) {
  ...
  const handleReject = async (id: number) => {
    ...
    if (!reason) return;
    try {
      await bookingApi.ownerReject(id, reason);
      await loadBookings();
    } catch (err) {
              ...
              {filteredBookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                    <Link to={`/owner/bookings/${booking.id}`} className="text-blue-600 hover:text-blue-900 mr-3">
                      View
                    </Link>
                    {booking.status === 'REQUESTED' && (
                      <>
                        <button onClick={() => handleApprove(booking.id)} className="text-green-600 hover:text-green-900 mr-3">
                          Approve
                        </button>
                        <button onClick={() => handleReject(booking.id)} className="text-red-600 hover:text-red-900">
                          Reject
                        </button>
                      </>
                    )}
